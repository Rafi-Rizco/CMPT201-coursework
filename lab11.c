...
int main() {
  ...
  FILE *file_pubkey = fopen("public_key.pem", "rb");
  if (!file_pubkey) {
    handle_error("fopen pubkey failed");
  }

  if (PEM_read_PUBKEY(file_pubkey, &pubkey, NULL, NULL) == NULL) {
    fclose(file_pubkey);
    return 1;
  }
...
#define MAX_FILE_SIZE 512
  ...
  int num_read = read_all_bytes(message_path, message, MAX_FILE_SIZE);
  int num_read2 = read_all_bytes(sign_path, signature, MAX_FILE_SIZE);
  ...
  if (EVP_DigestVerifyUpdate(mdctx, message, num_read) != -1) {
    EVP_MD_CTX_free(mdctx);
    return 0;
  }

  if (EVP_DigestVerifyFinal(mdctx, signature, num_read2) == 1) {
    EVP_MD_CTX_free(mdctx);
    return 1;
  }
